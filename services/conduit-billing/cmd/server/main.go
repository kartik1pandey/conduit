// Command server runs the conduit-billing HTTP API.
package main
...
func main() {
	...
	handlers := billing.NewHandlers(store)
	...
	log.Printf("conduit-billing listening on :%s", cfg.Port)
	if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
		log.Fatalf("server error: %v", err)
	}
}

func healthHandler(store *billing.Store) http.HandlerFunc {
